CREATE TABLE EFT.EXTERNAL_PAYMENT_TRANSACTION
(
...
LOGGING
NOCACHE;
COMMENT ON COLUMN EFT.EXTERNAL_PAYMENT_TRANSACTION.TRANSACTION_ID
    IS 'Identifiant unique de la transaction externe (clé primaire)';

COMMENT ON COLUMN EFT.EXTERNAL_PAYMENT_TRANSACTION.SOURCE_SYSTEM
    IS 'Système source ayant envoyé la transaction (ex: SystemA, SystemB)';

COMMENT ON COLUMN EFT.EXTERNAL_PAYMENT_TRANSACTION.TRANSACTION_DATE
    IS 'Date et heure réelle de la transaction (format TIMESTAMP)';

COMMENT ON COLUMN EFT.EXTERNAL_PAYMENT_TRANSACTION.JSON_PAYLOAD
    IS 'Payload JSON brut, validé par une contrainte JSON';

COMMENT ON COLUMN EFT.EXTERNAL_PAYMENT_TRANSACTION.SCHEMA_VERSION
    IS 'Version du schéma JSON pour permettre l’évolution du format';

COMMENT ON COLUMN EFT.EXTERNAL_PAYMENT_TRANSACTION.CREATED_AT
    IS 'Date de création de l’enregistrement dans le système';

COMMENT ON COLUMN EFT.EXTERNAL_PAYMENT_TRANSACTION.UPDATED_AT
    IS 'Date de la dernière mise à jour de l’enregistrement';

COMMENT ON COLUMN EFT.EXTERNAL_PAYMENT_TRANSACTION.STATUS
    IS 'Statut de traitement : NEW, PROCESSED, RECYCLE, REJECTED';


ALTER TABLE EFT.EXTERNAL_PAYMENT_TRANSACTION ADD CONSTRAINT PK_EXT_PAYMENT_TRANSACTION  PRIMARY KEY (TRANSACTION_ID) ;



CREATE OR REPLACE PUBLIC SYNONYM EXTERNAL_PAYMENT_TRANSACTION  FOR EFT.EXTERNAL_PAYMENT_TRANSACTION;
